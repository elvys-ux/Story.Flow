...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Guarda likes no localStorage
  let likedStories = JSON.parse(localStorage.getItem('likedStories') || '[]');
  let categoryMap  = {};

  // [1] Exibe usuário logado / link de login
  ...
  // [3] Busca histórias, cartões e categorias, monta allStories
  async function fetchStoriesFromSupabase() {
    ...
    // 3.4) Montagem do array allStories
  ...
  function formatarPor4Linhas(text) {
    if (!text) return '<p style="text-align: justify;">(sem conteúdo)</p>';
  ...
  function formatarTextoParaLeitura(text) {
    if (!text) return '<p>(sem conteúdo para leitura)</p>';
    const lines = text.split('\n');
    let idx = 0, paras = [], buf = [];
    lines.forEach((ln, i) => {
      buf.push(
