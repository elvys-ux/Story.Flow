...
/*************************************************************
 * HISTORIA.JS — Versão com Supabase (Atualizado)
 * - Remove LocalStorage
 * - Insere/atualiza/apaga dados nas tabelas Supabase
 * - Carrega categorias dinamicamente (id e nome)
 * - Ajusta event handlers para Cartão, Ler Mais e Voltar
 *************************************************************/

/************************************************************
 * Variáveis de contexto para Cartão
 ************************************************************/
...
/************************************************************
 * [4] EVENTO DOMContentLoaded + Binding de botões de Cartão
 ************************************************************/
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Form História: salvar, nova história, fechar modal
  ...
  // Botões de Cartão (sempre vinculados)
  ...
  btnPub?.addEventListener('click', () => {
    if (!storyIDAtual || !userIDAtual) {
      console.error('storyIDAtual ou userIDAtual não definidos');
      return;
    }
  ...
  btnLer?.addEventListener('click', () => {
    if (!storyIDAtual) {
      console.error('storyIDAtual não definido');
      return;
    }
...
function toggleMenuOpcoes(li, storyID) {
  ...
  ['Cartão','Editar','Excluir'].forEach(action => {
    ...
    btn.onclick =e => {
      ...
      if (action==='Excluir') excluirHistorias(storyID);
      menu.remove();
    };
...
/************************************************************
 * [12] CARTÃO (Supabase) — Ajustado para usar context globals
 ************************************************************/
async function mostrarCartaoForm(storyID) {
  // salva contexto
  storyIDAtual=storyID;
  const { data: { session }, error: sessErr } =await supabase.auth.getSession();
  if (sessErr||!session) { console.error('Usuário não autenticado'); return; }
...
async function publicarCartao(storyID,userId) {
  ...
  let upErr=(await supabase.from('cartoes').upsert({historia_id:storyID,titulo_cartao:tituloCartao,sinopse_cartao:sinopse,autor_cartao:autor,data_criacao:dataCri})).error;
  if(upErr) return alert('Erro ao salvar cartão: '+upErr.message);

  let delErr=(await supabase.from('historia_categorias').delete().eq('historia_id',storyID)).error;
  ...
  let insErr=(await supabase.from('historia_categorias').insert(inserts)).error;
