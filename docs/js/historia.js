...
let openMenu = null, menuTimeout = null;
...
// [4] Exibe histórias e cria <li> com onclick que chama showMenu
...
// Exibe menu flutuante, auto-fecha em 3s
...
async function mostrarCartaoForm(id) {
  ...
  // categorias do cartão usam mesma checklist
...
async function publicarCartao(id) {
  ...
  await supabase.from('cartoes').upsert({
    historia_id:id, titulo_cartao:titulo,
    sinopse_cartao:sinopse, autor_cartao:autor,
    data_criacao:dataC
  });
  const { data:{user} } = await supabase.auth.getUser();
  await supabase.from('historia_categorias').delete().eq('historia_id',id);
