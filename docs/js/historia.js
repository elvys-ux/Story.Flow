...
async function salvarHistoria(titulo, descricao) {
  ...
  if (editID) {
      ...
      // Insere nova história; o banco gera o ID automaticamente
...
async function mostrarCartaoForm(storyID) {
  ...
  // Atualiza os campos do formulário do cartão
  ...
  // O input date espera o formato yyyy-MM-dd – ajusta a data, se necessário
  document.getElementById('data_criacao').value = card 
      ? card.data_criacao 
      : new Date().toISOString().split('T')[0];
  document.getElementById('autor_cartao').value = card ? card.autor_cartao : "";

  // Se houver dados salvos de categorias, recupera o relacionamento
  let selectedCats = [];
  if (card) {
...
async function publicarCartao(storyID) {
  ...
  // Insere ou atualiza os dados do cartão
  ...
  // Para cada categoria selecionada, busca o ID e insere o relacionamento
  for (let categoriaName of categoriasSelecionadas) {
       ...
       if (catId) {
           ...
           if (catError) {
               ...
               console.log(`Relacionamento inserido para a categoria "${categoriaName}"`);
           }
       } else {
