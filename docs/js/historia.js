...
document.addEventListener('DOMContentLoaded', function() {
    /*************************************************************
     * VARIÁVEIS GLOBAIS – ARMAZENAMENTO COM localStorage
     *************************************************************/
    let modoCorrido = localStorage.getItem('modoCorrido') !== null 
        ? JSON.parse(localStorage.getItem('modoCorrido')) 
        : true;
    let partesHistoria = localStorage.getItem('partesHistoria') !== null 
        ? JSON.parse(localStorage.getItem('partesHistoria')) 
        : [];
    let parteAtual = localStorage.getItem('parteAtual') !== null 
        ? parseInt(localStorage.getItem('parteAtual'), 10) 
        : 0;
    ...
    async function exibirUsuarioLogado() {
        ...
        try {
            ...
            // Ao clicar, pergunta se deseja fazer logout e, se confirmado, efetua o logout e redireciona para Criacao.html.
            userArea.onclick = () => {
    ...
    /*************************************************************
     * FUNÇÃO: TOGGLE LEITURA – Alterna o modo de leitura e persiste no localStorage
     *************************************************************/
    ...
    async function exibirHistoriaNoContainer(storyID) {
        ...
        currentStoryId = storyID; // Define a história atual para o marcador de leitura
    ...
    async function filtrarHistorias(query) {
        ...
        // Para cada história, traz os dados do cartão ou define a partir do título
        const resultados = [];
    ...
    /*************************************************************
     * [N] EVENTO – CONFIGURAÇÃO INICIAL
     *************************************************************/
    // Configura o submit do formulário para salvar/editar a história
    const storyForm = document.getElementById('storyForm');
    if (storyForm) {
    ...
    // Configuração dos eventos para a lista lateral (hover)
    document.body.addEventListener('mousemove', function(e) {
        if (e.clientX < 50 && !isTitleListVisible) {
            toggleTitleList(true);
        }
